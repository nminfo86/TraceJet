...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        $AFTER_UPDATE = "DROP TRIGGER IF EXISTS AFTER_UPDATE_UPDATE_OF_STATUS;
                        CREATE TRIGGER UPDATE_OF_STATUS AFTER UPDATE ON serial_numbers
                        FOR EACH ROW BEGIN

                        IF ((SELECT COUNT(movements.id) FROM `movements` JOIN serial_numbers ON movements.serial_number_id=serial_numbers.id
                            WHERE serial_numbers.of_id=new.of_id) = 1) THEN
                                UPDATE ofs SET status='inProd',release_date=CURRENT_TIMESTAMP WHERE id=new.of_id;
                        END IF;
                        END";
