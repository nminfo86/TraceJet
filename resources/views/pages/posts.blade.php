                ...
                <form id="main_form">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-lg-3">
                                <label for="printer_id">{{ __('Imprimante') }}</label>
                                <select id="printer_id" name="printer_id"
                                    data-placeholder="{{ __('selectionner une imprimante') }}">
                                    <option value=" ">{{ __('------') }} </option>
                                </select>
                                <span class="invalid-feedback" role="alert">
                                    <strong id="printer_id-error"></strong>
                                </span>
                            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).on('change', "#section_id", (e) => {
            ...
            callAjax('GET', url + '/' + id).done(function(response) {
                ...
                $("#printer_id").val(response.data.printer_id).trigger('change');
        ...
        table = table.DataTable({
            ...
            columns: [{
                ...
                },
                {
                    data: 'id',
                    render: function(data, type, row) {
                        return `@can('post-edit') <div type="button" id="${data}" class="d-inline text-white edit"> <i class="fas fa-edit text-warning"></i></div> @endcan
                        @can('post-delete') <div type="button" id = ${data} class="d-inline pl-3 text-white delete"><i class="fas fa-trash text-danger"></i> </div> @endcan`;
                    }
                },
            ],
