...
class ProductService
{
    ...
    public function checkProductSteps($request, $last_movement)
    {
        ...
        $last_post = Post::findOrFail($last_movement->movement_post_id);

        // Check if the result of the last movement is NOK
        if ($last_movement->result == 'NOK') {
            if ($last_post->posts_type_id === 4) {
                return $this->sendResponse('Broken', status: false);
            }
            $msg = __('exception-errors.last_movement_nok', ['post_name' => $last_post->post_name]);
            return $this->sendResponse($msg, status: false);
        }
        ...
        if ($last_movement->movement_post_id != $request->previous_post_id) {
            ...
            // dd('raho makhdoum w raho arwah edih');
        }
    ...
    public function countValidProducts($of_id)
    {
        ...
        if (request()->posts_type == 1) {
            $movements = SerialNumber::whereOfId($of_id)
                ->whereValid(1)
                ->count();
        } else {
